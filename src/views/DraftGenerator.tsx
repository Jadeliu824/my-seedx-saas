...
export function DraftGenerator({ isMobile }: { isMobile?: boolean }) {
  ...
  const handleDeepen = async () => {
    ...
    const prompt = `
写作的最高标准不是漂亮，是真实。
好的内容让人读完之后觉得：这个人在说真话。
不是"这篇文章写得好"，而是"这说的是我"。

具体要求：
- 用数字和细节代替形容词。
  不是"囤积了很多截图"，而是"手机里存了1273张没打开过的截图"。
  不是"花了很多时间"，而是"屏幕使用时间这周是47小时"。
- 开头第一句，必须是一个具体的动作或场景，不是一个观点。
  对：「昨晚整理手机相册，发现一件小事。」
  错：「我们总是在消费，却从不生产。」
- 比喻最多用一个，用完就停。用了"仓鼠"就不要再用"水龙头"。
- 结尾的问题必须是读者今天就能回答的，不是开放性的人生问题。
  对：「你上一次把看到的东西转述给别人，是多久以前？」
  错：「你认为消费和生产的边界在哪里？」
